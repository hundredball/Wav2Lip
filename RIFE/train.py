...
def train(model, local_rank):
    ...
    step = 0
    nr_eval = 0
    dataset = LRS2Dataset('train')
    sampler = DistributedSampler(dataset)
    train_data = DataLoader(dataset, batch_size=args.batch_size, num_workers=8,
                            pin_memory=True, drop_last=True, sampler=sampler)
    args.step_per_epoch = train_data.__len__()
    dataset_val = LRS2Dataset('validation')
    ...
    for epoch in range(args.epoch):
        ...
        for i, data in enumerate(train_data):
            ...
            pred, merged_img, flow, loss_l1, loss_flow, loss_cons, loss_ter, flow_mask = model.update(
                imgs, gt, learning_rate, mul, True, flow_gt)
            ...
            if step % 10 == 1:
                print('epoch:{} {}/{} time:{:.2f}+{:.2f} loss_l1:{:.4e}'.format(epoch, i,
                                                                                args.step_per_epoch, data_time_interval, train_time_interval, loss_l1))
            step += 1
        nr_eval += 1
        if nr_eval % 5 == 0:
